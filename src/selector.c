...
Bool_t selector::Process()
{
	...
	for(Long64_t entry=0; entry<nentries; entry++)
	{
		...
		//      if(Runnr == 1 && Eventnr == 17116)
		{     
			//      if(Runnr == 52258 && Eventnr == 22172) check_cuts = kTRUE;//52258 22172
			//      else
			//        check_cuts = kFALSE;
			// check_cuts = kTRUE;
			...
			for(Int_t i=0; i<1; i++) {
				...
				//  
				if(Sith[i]*180.0/TMath::Pi() < 139.8 || Sith[i]*180.0/TMath::Pi() > 180.0) //warning
					//if(Sith[i]*180.0/TMath::Pi() < 139.8 || Sith[i]*180.0/TMath::Pi() > 171.9)
				{
					take_electron_event = kFALSE;
					if(check_cuts)
			...
			if(take_event)
			{
				...
				for(Int_t zloop=0; zloop<Nzufos; zloop++)
				{
					...
					if(use_ktjetb_prph && here_is_prph && zloop == max_et_candidate_number) {
						if(check_cuts) cout << "rejecting zufo number " << max_et_candidate_number << " with e = " << Zufo[zloop][3] << " because it is prph candidate " << endl;
						continue;
					}
				...
				//warning   KtEvent ev(input_vec,type,angle,recom,rparameter);

				// Retrieve the final state jets from KtEvent sorted by Et
				vector<KtLorentzVector> jets;//warning = ev.getJetsEt();
				...
				//      cout << "======================================================" << endl;
				//      cout << "after systematic shift:" << endl;
				//      for(Int_t jloop=0; jloop < jets.size(); jloop++)
				//        cout << jets[jloop].px() << " " << jets[jloop].py() << " " << jets[jloop].pz() << " " << jets[jloop].e() << " " << jets[jloop].et() <<endl;
				//
				...
				if(take_event && here_is_jet && here_is_prph) {
					...
					if(take_event_trig)
					{
						...
						hist.phjet_dphi->Fill(photon_jet_dphi, wtx);
