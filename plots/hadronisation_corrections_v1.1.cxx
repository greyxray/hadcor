...
int main(int argc, char *argv[])
{
	...
	cout << "============ Errors ================" << endl;
	for(Int_t i=0; i<hist_had_to_part[i]->GetNbinsX(); i++) {
		cout << "bin numerator " << i << ": " << hist_had[0]->GetBinContent(i+1) << " +- " << hist_had[0]->GetBinError(i+1) << endl;
		cout << "bin denomenator " << i << ": " << hist_part[0]->GetBinContent(i+1) << " +- " << hist_part[0]->GetBinError(i+1) << endl;
		cout << "bin ratio " << i << ": " << hist_had_to_part[0]->GetBinContent(i+1) << " +- " << hist_had_to_part[0]->GetBinError(i+1) << endl;
	...
	TH2D *h_window2[6];
	h_window2[0] = new TH2D("h_window2_et", "", number_etbins, et_bin, 10, 0.1, 1.25 * hist_part[0]->GetMaximum());
	h_window2[1] = new TH2D("h_window2_eta", "", number_etabins, eta_bin_crosssec, 10, 0.1, 1.25 * hist_part[1]->GetMaximum());
	h_window2[2] = new TH2D("h_window2_Q2", "", number_Q2bins, Q2_bin, 10, 0.1, 1.25 * hist_part[2]->GetMaximum());
	h_window2[3] = new TH2D("h_window2_x", "", number_xbins, x_bin, 10, 0.1, 1.25 * hist_part[3]->GetMaximum());
	h_window2[4] = new TH2D("h_window2_et_jet", "", number_et_jetbins, et_jet_bin, 10, 0.1, 1.25 * hist_part[4]->GetMaximum());
	h_window2[5] = new TH2D("h_window2_eta_jet", "", number_eta_jetbins, eta_jet_bin, 10, 0.1, 1.25 * hist_part[5]->GetMaximum());
	...
	for(Int_t i=0; i<6; i++)
		sign_window(c2->GetPad(i+1), h_window2[i], s_dim[i], "Events / bin width", "", "middle");

	c2->GetPad(3)->SetLogx();
	c2->GetPad(4)->SetLogx();
	c2->GetPad(5)->SetLogx();

	TLegend *leg2 = new TLegend(0.3, 0.68, 0.9, 0.93, "LL Ariadne");
	...
	for(Int_t i=0; i<n_hist; i++){
		c2->GetPad(i+1)->cd();
		h_window2[i]->DrawClone();
		hist_part[i]->SetLineColor(kRed);
		hist_part[i]->DrawClone("][ HIST SAME");
		hist_part[i]->SetFillColor(kRed);
		hist_part[i]->SetFillStyle(3335);
		hist_part[i]->DrawClone("][ E2 SAME");
		hist_had[i]->SetLineColor(kBlue);
		hist_had[i]->DrawClone("][ HIST SAME");
		hist_had[i]->SetFillColor(kBlue);
		hist_had[i]->SetFillStyle(3353);
		hist_had[i]->DrawClone("][ E2 SAME");
		if(i==0) leg2->Draw();
	}

	c2->Print("had_part_LL.eps");
