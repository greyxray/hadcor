...
void set_hist_atributes(Int_t number, TString name, TString title, TString xaxis, TString yaxis, Double_t xmin, Double_t xmax, Double_t ymin, Bool_t logy, Bool_t logx, Int_t rebin);
void set_hist_atributes(Int_t number, TString name, TString title, TString xaxis, TString yaxis, Double_t xmin, Double_t xmax, Double_t ymin, Double_t ratio_ymin, Double_t ratio_ymax, Bool_t logy, Bool_t logx, Int_t rebin);

const Int_t n_hist = 51;
...
Double_t hist_ratio_ymin[n_hist];
Double_t hist_ratio_ymax[n_hist];
Bool_t hist_logy[n_hist];
Bool_t hist_logx[n_hist];
...
int main(int argc, char *argv[])
{
  ...
  set_hist_atributes(40, "h_had_ijet_q2", "inclusive jets, had. level, log scale", "Q^{2} (GeV^{2})", "Jets", 0., 100., 0.1, kTRUE, 10);
  set_hist_atributes(41, "h_had_ijet_q2", "inclusive jets, had. level", "Q^{2} (GeV^{2})", "Jets", 0., 100., 0.1, kFALSE, 10);
  set_hist_atributes(42, "h_had_ijet_x", "inclusive jets, had. level, log scale", "x", "Jets", 1.e-5, 5.e-3, 0.1, kTRUE, 10);
  set_hist_atributes(43, "h_had_ijet_x", "inclusive jets, had. level", "x", "Jets", 1.e-5, 5.e-3, 0.1, kFALSE, 10);

  set_hist_atributes(44, "h_had_ijet_pt_breit", "inclusive jets, had.level, log scale", "P_{T,Breit} (GeV)", "Jets", 0., 120., 0.1, 0., 3., kTRUE, kFALSE, 10);
  set_hist_atributes(45, "h_had_ijet_pt_breit", "inclusive jets, had.level", "P_{T,Breit} (GeV)", "Jets", 0., 120., 0.1, 0., 3., kFALSE, kFALSE, 10);
  set_hist_atributes(46, "h_had_ijet_pt_lab", "inclusive jets, had.level, log scale", "P_{T,Lab} (GeV)", "Jets", 0., 120., 0.1, 0., 2.2, kTRUE, kFALSE, 10);
  set_hist_atributes(47, "h_had_ijet_pt_lab", "inclusive jets, had.level", "P_{T,Lab} (GeV)", "Jets", 0., 120., 0.1, 0., 2.2, kFALSE, kFALSE, 10);

  set_hist_atributes(48, "h_had_ijet_y", "inclusive jets, had.level", "y", "Jets", 0.1, 0.7, 0.1, kFALSE, 10);
  set_hist_atributes(49, "h_had_ijet_eta_lab", "inclusive jets, had.level", "#eta_{Lab}", "Jets", -1., 2.5, 0.1, 0.8, 1.3, kFALSE, kFALSE, 10);
  set_hist_atributes(50, "h_had_ijet_eta_breit", "inclusive jets, had.level", "#eta_{Breit}", "Jets", 0., 5., 0.1, 0.5, 1.5, kFALSE, kFALSE, 10);

  //  set_hist_atributes(44, "h_had_ijet_x", "inclusive jets, had. level", "x", "Jets", 1.e-5, 5.e-3, 0.1, kTRUE, 10);
  //  set_hist_atributes(46, "h_had_ijet_eta_lab", "inclusive jets, had.level", "#eta_{Lab}", "Jets", -1., 2.5, 0.1, 0.8, 1.3, kFALSE, kFALSE, 10);
  //  set_hist_atributes(47, "h_had_ijet_pt_lab", "inclusive jets, had.level", "P_{T,Lab} (GeV)", "Jets", 0., 120., 0.1, 0., 2.2, kTRUE, kFALSE, 10);
...
//   set_hist_atributes(76, "h_part_ijet_jetel_px_balance", "#Sigma_{jets, el.} P_{x, part}", "P_{x, sum}^{part} (GeV)", "Events", -50., 50., 0.1, kTRUE, 1);
//   set_hist_atributes(77, "h_part_ijet_jetel_py_balance", "#Sigma_{jets, el.} P_{y, part}", "P_{y, sum}^{part} (GeV)", "Events", -50., 50., 0.1, kTRUE, 1);
//   set_hist_atributes(78, "h_part_ijet_jetel_pz_balance", "#Sigma_{jets, el.} P_{z, part}", "P_{z, sum}^{part} (GeV)", "Events", -1000., 1000., 0.1, kTRUE, 1);
//   set_hist_atributes(79, "h_part_ijet_jetel_e_balance", "#Sigma_{jets, el.} E_{part}", "E_{sum}^{part} (GeV)", "Events", -1000., 1000., 0.1, kTRUE, 1);

//   set_hist_atributes(80, "h_part_dis_px_sum", "inclusive DIS, part. level", "P_{x, sum}^{part} (GeV)", "Events", -10., 10., 0.1, kTRUE, 1);
//   set_hist_atributes(81, "h_part_dis_py_sum", "inclusive DIS, part. level", "P_{y, sum}^{part} (GeV)", "Events", -10., 10., 0.1, kTRUE, 1);
//   set_hist_atributes(82, "h_part_dis_pz_sum", "inclusive DIS, part. level", "P_{z, sum}^{part} (GeV)", "Events", -1000., 100., 0.1, kTRUE, 1);
//   set_hist_atributes(83, "h_part_dis_e_sum", "inclusive DIS, part. level", "E_{sum}^{part} (GeV)", "Events", -1000., 100., 0.1, kTRUE, 1);
...
// 			     "h_part_px", "h_part_py", "h_part_pz", "h_part_e"  
  ...
  //  TFile *file1 = new TFile("/data/zenith234b/kuprash/analyses/jets_low_q2/0607p_v04b_ari_h1.root", "read"); 
  //  TFile *file2 = new TFile("/data/zenith234b/kuprash/analyses/jets_low_q2/0607p_v04b_ari_orange.root");
  TFile *file1 = new TFile("/data/zenith234b/kuprash/analyses/jets_low_q2/06e_ari_h1.root", "read"); 
  TFile *file2 = new TFile("/data/zenith234b/kuprash/analyses/jets_low_q2/06e_ari_orange.root");
  ...
  for(Int_t i=0; i<n_hist; i++) {
    ...
    h_window[i] = new TH2D("h_window" + s_hist1[i], "title", 10, hist_xmin[i], hist_xmax[i], 10, hist_ymin[i], offset * hist1[i]->GetMaximum());
    h_window_ratio[i] = new TH2D("h_window" + s_hist1[i] + "_ratio", "", 10, hist_xmin[i], hist_xmax[i], 10, hist_ratio_ymin[i], hist_ratio_ymax[i]);
  }
  ...
  for(Int_t i=0; i<n_hist; i++) {
    ...
    c1[i/4]->GetPad(i%4+1)->GetPad(1)->SetLogx(hist_logx[i]);
    c1[i/4]->GetPad(i%4+1)->GetPad(2)->SetLogx(hist_logx[i]);
  }
  
  TLegend *leg = new TLegend(0.2, 0.75, 0.85, 0.89);
  leg->SetBorderSize(0);
  leg->SetFillColor(0);
...
//   c1[5]->GetPad(4)->SetLogy();
  ...
  for(Int_t i=0; i<n_hist/4+1; i++) {
    TString s; s.Form("comp_new_common%i_norm.eps", i);
    c1[i]->Print(s);
  }
  //  c1->Print("Q2.png");
  return 0;
...
void set_hist_atributes(Int_t number, TString name, TString xaxis, TString yaxis, Double_t xmin, Double_t xmax, Double_t ymin, Bool_t logy, Int_t rebin)
{
  ...
  hist_logx[number] = kFALSE;
  hist_ratio_ymin[number] =  0.8;
  hist_ratio_ymax[number] =  1.2;
}
...
void set_hist_atributes(Int_t number, TString name, TString title, TString xaxis, TString yaxis, Double_t xmin, Double_t xmax, Double_t ymin, Double_t ratio_ymin, Double_t ratio_ymax, Bool_t logy, Bool_t logx, Int_t rebin)
{
  ...
  hist_logx[number] = logx;
  hist_rebin[number] = rebin;
  hist_ratio_ymin[number] =  ratio_ymin;
  hist_ratio_ymax[number] =  ratio_ymax;
}
